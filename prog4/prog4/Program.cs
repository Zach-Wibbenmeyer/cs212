...
/* CS212 -Data Structures and Algorithms, Program 4
        ...
        private static void CommandLoop()
        {
            ...
            while (command != "exit")
            {
                ...
                else if (command == "bingo")
                {
                    ...
                    var path = rg.FindShortestPath(commandWords[1], commandWords[2]);
                    if (path != null)
                    {
                        for (var i = 1; i < path.Count; i++)
                        {
                            foreach (var e in path[i - 1].GetEdges())
                            {
                                if (e.ToNode() == path[i])
                ...
                {
                    rg.FindDescendants(commandWords[1]);
                }

                // FindCousins
                else if (command == "cousins")
                {
                    rg.FindCousins(commandWords[1], int.Parse(commandWords[2]), int.Parse(commandWords[3]));
                }
